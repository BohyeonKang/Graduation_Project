        ...
        for(row=0; row<NUM_ROWS; row=row+1) begin
            for(col=0; col<NUM_COLS; col=col+1) begin
                always @(*) begin
                    ...
                    ctrl_psum_in_sel_LNorGIN[row][col] = (psum_row_id[row][col] == i_layer_s);
                    ctrl_psum_out_sel_GON[row][col] = (psum_row_id[row][col] == 1'd1);
                end
                assign o_ctrl_psum_in_sel_LNorGIN[row * NUM_COLS + col] = ctrl_psum_in_sel_LNorGIN[row][col];
                assign o_ctrl_psum_out_sel_GON[row * NUM_COLS + col] = ctrl_psum_out_sel_GON[row][col];
            end 
        end
    endgenerate
endmodule
