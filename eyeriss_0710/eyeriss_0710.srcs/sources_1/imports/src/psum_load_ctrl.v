...
module psum_load_ctrl(
    ...
    assign o_psum_glb_ra = (cnt_P * i_layer_EF * i_layer_EF) + (cnt_E * i_layer_EF) + cnt_F;
    ...
    always @(*) begin
        case(state)
            ...
            UPDATE_BASE: n_state = (pass_done) ? DONE : IDLE;
            DONE:     n_state = IDLE;
        ...
        if (i_rst) begin
            ...
            if (cnt_P == i_layer_p - 1) begin
                ...
                if (cnt_E == i_layer_EF - 1) begin
                    cnt_E <= 0;
                    if (cnt_F == i_layer_EF - 1) begin
                        cnt_F <= 0;
                        if (cnt_t == i_layer_t - 1) cnt_t <= 0;
                        else cnt_t <= cnt_t + 1;
                    end 
                    else cnt_F <= cnt_F + 1;
                end 
