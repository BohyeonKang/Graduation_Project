...
class PE:
    def __init__(self, pe_id):
        self.pe_id = pe_id
        # 모든 spad를 정수형으로 초기화 (Python의 int는 임의 정밀도를 지원)
    ...
    def perform_1d_convolution(self):
        ...
        print(f"  [{' '.join(map(str, self.ifmap_spad.flatten()))}]")
...
class GLB:
    ...
    def initialize_banks(self):
        # Bank0: ifmap 더미 데이터 (INT8 signed 범위: -128 ~ 127)
        # np.random.randint(low, high, size) -> low <= N < high
        self.bank0_ifmap = np.random.randint(-128, 128, size=(IFMAP_C * IFMAP_H * IFMAP_W)).reshape(IFMAP_C, IFMAP_H, IFMAP_W).transpose(1, 2, 0).astype(int)
        print("\n--- GLB Bank0 (ifmap) 초기화 완료 (INT8 signed garbage value, 채널별 시각화) ---")
        for c in range(IFMAP_C):
            print(f"--- ifmap Channel {c} ---")
            print(self.bank0_ifmap[:, :, c])


        # Bank1: weights 더미 데이터 (INT8 signed 범위: -128 ~ 127)
        self.bank1_weights = np.random.randint(-128, 128, size=(OUTPUT_C * FILTER_C_IN * FILTER_H * FILTER_W)).reshape(OUTPUT_C, FILTER_C_IN, FILTER_H, FILTER_W).astype(int)
        print("\n--- GLB Bank1 (weights) 초기화 완료 (INT8 signed garbage value, 필터별/채널별 시각화) ---")
...
output_filename = "convolution_simulation_output_int8.txt" # 파일명 변경
...
try:
    with open(output_filename, 'w') as f:
        ...
        print("--- RS Dataflow 기반 3D Convolution 검증 시뮬레이션 시작 (INT8 Signed Data) ---")
